void get_weather(int type) {
  memset(request_buffer, NULL, 1); //reblank the buffer
  memset(response_buffer, NULL, 1); //reblank the buffer
  sprintf(request_buffer, "GET http://ireneterpstra.pythonanywhere.com/?type=%d&loc=Boston HTTP/1.1\r\n", type); //CHANGE CITY HERE
...
//  bool connectionstat = false;
  ...
  Serial.println(response_size);
  Serial.println("response_timeout --");
  Serial.println(response_timeout);
